...
async def decrypt(encrypted_data, key, session):
    try:
        ...
        await send_message_to_oled("Entry Error: Invalid QR Code", session)
...
async def fetch_terminal_fee(barangay):
    try:
        # Map barangay to terminal fee document
        barangay_mapping = {
            "Barandal": "barandal-terminal-fee",
            "Bubuyan": "bubuyan-terminal-fee",
            "Bunggo": "bunggo-terminal-fee",
            "Burol": "burol-terminal-fee",
            "Kay-anlog": "kay-anlog-terminal-fee",
            "Prinza": "prinza-terminal-fee",
            "Punta": "punta-terminal-fee"
        }

        barangay_fee = barangay_mapping.get(barangay)
        if barangay_fee:
            # Fetch the terminal fee for the specific barangay
            fee_ref = db.collection("dashboard-counts").document(barangay_fee)
            fee_doc = fee_ref.get()
            if fee_doc.exists:
                return fee_doc.to_dict().get("fee")
        return None
    except Exception as e:
...
async def join_queue(user_data, session):
    ...
    try:
        ...
        terminal_fee = await fetch_terminal_fee(user_data.get("barangay"))
        ...
        # end_time = time.time()
        # total_latency = (end_time - start_time) * 1_000  # Convert to ms
        # print(f"Total latency for join queue: {total_latency:.2f} ms")
