...
#define BUZZER_PIN 4 // Define the pin number connected to the buzzer
...
// Buzzer handler function for the /buzzer route
static esp_err_t buzzer_handler(httpd_req_t *req) {
    ...
    const char* resp_str = "Buzzer activated!";
...
void init_lcd() {
    // Start the I2C bus with the defined SDA and SCL pins
...
// Function to display the message on LCD
void display_message(const char *message) {
    ...
    // Check if the message fits in the first row (assuming 16 characters per row)
    if (messageLength <= 16) {
        lcd.print(message);  // Print the whole message on the first line
    } else {
        ...
        // Print the remaining message on the second line
        for (int i = 16; i < messageLength && i < 32; i++) {  // Ensure not printing beyond 32 chars (16x2)
            lcd.print(message[i]);  
        }
    }
}
...
esp_err_t display_message_handler(httpd_req_t *req) {
    char buf[100];
    int ret;

    // Read the message content from the POST request
    if ((ret = httpd_req_recv(req, buf, sizeof(buf))) >= 0) {
        ...
        httpd_resp_send(req, "Message displayed", HTTPD_RESP_USE_STRLEN);
        return ESP_OK;
    }

    // Respond with an error if message processing failed
    httpd_resp_send_500(req);
    return ESP_FAIL;
}
...
void startCameraServer() {
  ...
  // URI configuration for the buzzer activation
  httpd_uri_t buzzer_uri = {
    .uri = "/activate_buzzer",        // The route for buzzer activation
    .method = HTTP_GET,               // The HTTP method (GET)
    .handler = buzzer_handler,        // Handler function
    .user_ctx = NULL                  // No additional user context
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,             // WebSocket support (optional)
    .handle_ws_control_frames = false,
    .supported_subprotocol = NULL
#endif
};

  // URI configuration for displaying message on OLED
  httpd_uri_t display_message_uri = {
    .uri = "/display_message",    // The route for displaying message on OLED
    .method = HTTP_POST,          // The HTTP method (POST to send data)
    .handler = display_message_handler,  // Handler function
    .user_ctx = NULL                  // No additional user context
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,             // WebSocket support (optional)
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &display_message_uri);
